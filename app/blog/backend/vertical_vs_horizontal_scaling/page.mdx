...
As backend engineers, we spend much of our time designing systems that can handle growth—more users, more traffic, more data. It is easy to say "we'll scale" without being clear *how*: add more CPU and RAM to the same machine, or add more machines. The first is <Term>vertical scaling</Term> (scale up); the second is <Term>horizontal scaling</Term> (scale out). Each has different costs, limits, and implications for architecture and operations.
...
<Term variant="neutral">Vertical scaling (scale up)</Term>
...
<Term variant="neutral">Horizontal scaling (scale out)</Term>
